...
class TripRepository {
    ...
    */
    ...
    fun searchTripByDateRange(searchQuery: SearchQuery){
        if (tripList.isNotEmpty()){
            val dateRangeList = searchQuery.dateRange
            //Assuming only 2 dates would be there
            if (dateRangeList?.size != 2){
                throw Exception("Date range is not valid")
            } else{
                val startDate = dateRangeList.get(0)
                val endDate = dateRangeList.get(1)

                val searchResults = tripList.filter {
                    it.startDate in startDate..endDate
                }
                println("Search results for dateRange = $dateRangeList are trips = $searchResults")
            }
        } else{
            throw Exception("No trips created yet!")
        }
    ...
    fun addTripToFavouritesById(id: Int){
        ...
        if (trip != null) {
            ...
            //updating the list at that idx after changing the value of isFavouriteTrip
